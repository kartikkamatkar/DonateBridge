...
export default function RequestWizard() {
   ...
   navigator.geolocation.getCurrentPosition(
    async (position) => {
     ...
     try {
      ...
      if (data.display_name) setAddressSearch(data.display_name);
     } catch (e) {
      console.error(e);
     }
    },
 ...
 const handleMapClick = async (latlng) => {
  ...
  setSelectedCoords({ lat, lng });
  toast.success(`Pickup point pinned: ${lat.toFixed(5)}, ${lng.toFixed(5)}`);
  try {
   const res = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}`);
   const data = await res.json();
   if (data.display_name) setAddressSearch(data.display_name);
        ...
        <div className="space-y-3">
         ...
         <div className="flex gap-2">
          <input
           ...
           className="flex-1 px-4 py-3 border border-border rounded-xl text-xs bg-slate-50 text-slate-900 focus:outline-none focus:border-primary placeholder-slate-400 font-medium"
          />
          <button
           type="button"
           onClick={handleAddressSearch}
           className="px-4 py-2 border border-slate-200 bg-white hover:bg-slate-50 font-bold rounded-xl cursor-pointer transition-colors text-xs"
          >
           Search Location
          </button>
         </div>
          ...
          <ul className="border border-border rounded-xl bg-white text-xs max-h-36 overflow-y-auto divide-y divide-border">
           {searchResults.map((item, i) => (
            <li
             ...
             className="p-3 hover:bg-slate-50 cursor-pointer truncate font-medium text-slate-700"
            >
             {item.display_name}
            </li>
           ))}
