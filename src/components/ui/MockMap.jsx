...
export const MockMap = ({
  ...
  return (
    <div className={`relative bg-slate-900 border border-slate-700 rounded-lg overflow-hidden flex flex-col ${className}`}>
      {/* Header Info Panel */}
      <div className="absolute top-3 left-3 right-3 bg-slate-950/80 backdrop-blur-md border border-slate-800 rounded px-3 py-2 flex items-center justify-between text-xs z-10 text-white">
        <div className="flex items-center gap-2">
          <Navigation className="w-4 h-4 text-emerald-400 animate-pulse" />
          <div>
            <p className="font-semibold text-emerald-400">Live Logistics Stream</p>
        ...
        <div className="flex items-center gap-3">
          ...
          <RefreshCw className="w-3.5 h-3.5 text-slate-400 hover:text-emerald-400 cursor-pointer transition-colors" />
        </div>
      </div>

      {/* SVG Canvas Map */}
      <div className="flex-1 w-full relative min-h-[300px] bg-slate-950">
        <svg viewBox="0 0 500 400" className="w-full h-full">
          ...
          <g stroke="rgba(71, 85, 105, 0.3)" strokeWidth="4" strokeLinecap="round" fill="none">
            {/* Horizontal Road */}
            ...
            <path d="M 100 150 L 320 280" stroke="rgba(16, 185, 129, 0.4)" strokeWidth="3" />
          ...
          {/* Locations & Markers */}
          {/* Donor Pin */}
          <circle cx="100" cy="150" r="15" fill="rgba(59, 130, 246, 0.2)" />
          <circle cx="100" cy="150" r="5" fill="#3b82f6" />
          <text x="100" y="130" fill="#93c5fd" fontSize="10" textAnchor="middle" fontWeight="bold">Donor</text>
          ...
          {Object.values(locations).filter(l => l.type === 'ngo').map((ngo) => {
            ...
            return (
              <g key={ngo.id} className="cursor-pointer">
                ...
                <circle cx={ngo.x} cy={ngo.y} r="14" fill={isHighlighted ? '#10b981' : '#334155'} />
                <path
                  d="M 12,2 A 8,8 0 0,0 4,10 c 0,6 8,12 8,12 0,0 8,-6 8,-12 A 8,8 0 0,0 12,2 Z"
                  fill={isHighlighted ? '#047857' : '#1e293b'}
                  transform={`translate(${ngo.x - 12}, ${ngo.y - 15}) scale(1)`}
                />
                <circle cx={ngo.x} cy={ngo.y} r="3" fill="#ffffff" />
                <text x={ngo.x} y={ngo.y - 20} fill={isHighlighted ? '#34d399' : '#94a3b8'} fontSize="9" textAnchor="middle" fontWeight="semibold">
          ...
          {(activeStep === 2 || activeStep === 3) && (
            <g transform={`translate(${courierPos.x - 10}, ${courierPos.y - 10})`}>
              <circle cx="10" cy="10" r="8" fill="#eab308" className="animate-ping opacity-75" />
              <rect x="2" y="2" width="16" height="16" rx="4" fill="#eab308" />
              <path
      ...
      {/* Footer Details */}
      <div className="bg-slate-950 border-t border-slate-800 p-3 flex justify-between items-center text-xs text-white">
        <div className="flex items-center gap-2">
          <Truck className="w-4 h-4 text-emerald-400" />
          <div>
            <span className="text-slate-400 block text-[9px]">ACTIVE COURIER</span>
            <span className="font-semibold text-slate-200">Express Cargo #DB-990</span>
          </div>
        </div>
        <div className="text-right">
          <span className="text-slate-400 block text-[9px]">LOGISTICS HUB</span>
          <span className="font-semibold text-slate-200">District Center 4B</span>
        </div>
      </div>
    </div>
  );
